...
CREATE TABLE IF NOT EXISTS IssuedBooks(
        ...
        FOREIGN KEY (reg_id) REFERENCES Students(reg_id) ON DELETE CASCADE
);
...
INSERT INTO students VALUES (116, 'Vinod Khanna', 'IT', 'BTech', 2023);
...
INSERT INTO IssuedBooks VALUES (990, '2020-04-10', '2022-04-19', 10, 116);
...
WHERE  DATEDIFF(NOW(), ISSUEDBOOKS.issue_date)>=14 AND ISSUEDBOOKS.return_date IS NULL;
...
-- ON STUDENTS.reg_id = ISSUEDBOOKS.reg_id
-- WHERE TIMESTAMPDIFF(YEAR, ISSUEDBOOKS.issue_date, NOW()) >=3;
...
WHERE reg_id IN (
    ...
    FROM (
        ...
        FROM ISSUEDBOOKS
        GROUP BY reg_id
        HAVING TIMESTAMPDIFF(YEAR, MAX(issue_date), NOW()) >= 3
    ) AS temp
);
